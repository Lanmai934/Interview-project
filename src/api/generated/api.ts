...
export class ApiClient {
   ...
   * 用户登录
   */
  async userLogin(data: any): Promise<AxiosResponse<any>> {
   ...
   * 获取用户列表
   */
  async getUserList(page?: number, limit?: number, status?: 'active' | 'inactive'): Promise<AxiosResponse<any>> {
    const params = {
      page,
      limit,
      status,
   ...
   * 获取用户详情
   */
  async getUserById(id: number): Promise<AxiosResponse<any>> {
   ...
   * 删除用户
   */
  async deleteUser(id: number): Promise<AxiosResponse<any>> {
